...
class TrajectoryDataset(Dataset):
    ...
    def _inspect_file(
        self,
        path: Path,
    ) -> Tuple[str, int, List[FieldInfo], List[np.ndarray]]:
        with h5py.File(path, "r") as handle:
            n_spatial_dims = int(handle.attrs.get("n_spatial_dims", 2))
            dataset_name = str(handle.attrs.get("dataset_name", "unknown"))
        ...
        return dataset_name, n_spatial_dims, field_infos, space_axes
